...
/**
 * Controller for Jsch SSH sessions. All SSH connections are run through this
 * class.
 */
public class SessionController {
  ...
  /**
   * Sets the user info for Session connection. User info includes username,
   * hostname and user password.
   *
  ...
  /**
   * Execute command on remote server. If SSH is not open, SSH shell will be
   * opened and command executed.
   *
   * @param command command to execute on remote host.
   * @return command sent true, if not false
   */
  public boolean executeCommand(Handler handler, ExecTaskCallbackHandler callback, String command) {
    ...
    if (mSession == null || !mSession.isConnected()) {
      ...
      try {
        mShellController.openShell(getSession(), callback, command);
      }
      catch (Exception e) {
  ...
  /**
   * Runnable for beginning session. Opens JSch session with username, password
   * and host information from <b>mSessionUserInfo</b>.
   */
  public class SshRunnable implements Runnable {
    public void run() {
      ...
      mSession = null;
      try {
        mSession = jsch.getSession(mSessionUserInfo.getUser(), mSessionUserInfo.getHost(), mSessionUserInfo.getPort()); // port 22
          ...
          public void run() {
            while (true) {
              try {
                Thread.sleep(2000);
                if (mConnectStatusListener != null) {
                  if (mSession.isConnected()) mConnectStatusListener.onConnected();
        ...
        if (mConnectStatusListener != null) mConnectStatusListener.onDisconnected();
      }
      catch (Exception ex) {
